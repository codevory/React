...
const Posts = ({posts}) => {
  ...
  return ( posts.map((post) => {
     ...
     <Link to={`/article/${post.id}`}>
       <div className=' flex flex-col w-full min-h-40 bg-transparent border-2 rounded-[7px] px-2 py-1 '>
          <div className='flex justify-between'>
        <span className='text-[18px] font-semibold md:text-[19px]'> {post.title}</span>
         <span className='text-[14px] font-semibold md:text-[17px] rounded-lg md:p-1'>{post.category}</span>
          </div>
